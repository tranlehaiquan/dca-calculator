...
interface VnResultsDashboardProps {
  result: InvestmentResult;
  symbol: string;
}

export const VnResultsDashboard = memo(function VnResultsDashboard({
  result,
  symbol,
}: VnResultsDashboardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <Card className="border-blue-500/20 bg-gradient-to-br from-blue-500/10 to-transparent transition-transform hover:-translate-y-1">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-muted-foreground flex items-center gap-2 text-sm font-medium">
              <Coins size={16} className="text-blue-500" />
              {t("results.value_in", { asset: symbol })}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {result.totalUnits.toFixed(2)} cổ phiếu
            </div>
