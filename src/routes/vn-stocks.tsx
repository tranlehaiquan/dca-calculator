...
import { useVnStockCalculator } from "../hooks/useVnStockCalculator";
import { VnStockInputForm } from "../components/VnStockInputForm";
import { VnResultsDashboard } from "../components/VnResultsDashboard";
import { VnTransactionHistory } from "../components/VnTransactionHistory";
import { VnChart } from "../components/VnChart";
import { SEO } from "../components/SEO";
import { TrendingUp, PanelLeftClose, PanelLeftOpen } from "lucide-react";
import { useTranslation } from "react-i18next";
import { Button } from "../components/ui/button";

export const Route = createFileRoute('/vn-stocks')({
  component: VnStocksComponent,
});

function VnStocksComponent() {
  const { t } = useTranslation();
  const [isSidebarVisible, setIsSidebarVisible] = useState(true);
  const dca = useVnStockCalculator();

  return (
    <div>
      <SEO 
        title={`${t("vn_stocks.title")} - DCA Calculator`}
        description="Calculate your Dollar Cost Averaging (DCA) returns for Vietnamese stocks like VNM, FPT, HPG, etc."
      />
      ...
      <main>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <section className={`space-y-8 ${isSidebarVisible ? "lg:col-span-8" : "lg:col-span-12"}`}>
            {dca.result ? (
              <>
                <VnResultsDashboard result={dca.result} symbol={dca.symbol} />
                <VnChart data={dca.result.history} symbol={dca.symbol} />
                <VnTransactionHistory 
                  result={dca.result} 
                  symbol={dca.symbol} 
                />
              </>
